...
TEST_CASE("percent_decode", "[percent_decode]") {
  ...
  SECTION("decode_codepoints_set") {
    for (auto i = 0x00; i < 0xff; ++i) {
      auto decoded = skyr::percent_encoding::as<std::string>(
          fmt::format("%{:02X}", i) | skyr::percent_encoding::views::decode);
      REQUIRE(decoded);
  ...
  SECTION("u8_test") {
    auto decoded = skyr::percent_encoding::as<std::string>(
        "%F0%9F%8F%B3%EF%B8%8F%E2%80%8D%F0%9F%8C%88"s | skyr::percent_encoding::views::decode);
    REQUIRE(decoded);
  ...
  SECTION("empty string") {
    auto decoded = skyr::percent_encoding::as<std::string>(
        ""s | skyr::percent_encoding::views::decode);
    CHECK(decoded);
  }
}
