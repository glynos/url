...
TEST_CASE("encode path", "[percent_encoding]") {
  ...
  SECTION("encode_path_set") {
    auto encoded = skyr::percent_encoding::percent_encode_byte(std::byte(c), skyr::percent_encoding::encode_set::path);
    CHECK(encoded.is_encoded());
  }
}
...
TEST_CASE("encode userinfo", "[percent_encoding]") {
  ...
  SECTION("encode_userinfo_set") {
    auto encoded =
        skyr::percent_encoding::percent_encode_byte(std::byte(c), skyr::percent_encoding::encode_set::userinfo);
    CHECK(encoded.is_encoded());
  }
}
...
TEST_CASE("encode_tests", "[percent_encoding]") {
  ...
  SECTION("encode_codepoints_before_0x20_set") {
    for (auto i = 0u; i < 0x20u; ++i) {
      auto encoded =
          skyr::percent_encoding::percent_encode_byte(std::byte(i), skyr::percent_encoding::encode_set::c0_control);
  ...
  SECTION("encode_0x25") {
    auto encoded =
        skyr::percent_encoding::percent_encode_byte(std::byte(0x25), skyr::percent_encoding::encode_set::any);
    CHECK("%25" == encoded.to_string());
  }

  SECTION("encode_0x2b") {
    auto encoded =
        skyr::percent_encoding::percent_encode_byte(std::byte(0x2b), skyr::percent_encoding::encode_set::any);
