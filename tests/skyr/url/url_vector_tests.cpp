...
TEST_CASE("url_vector_tests", "[url]") {
  SECTION("vector") {
    std::vector<skyr::url> urls{skyr::url("http://www.example.com/"), skyr::url("http://www.example.org/")};

    CHECK(urls.size() == 2);
  }

  SECTION("sorted_vector") {
    std::vector<skyr::url> urls{skyr::url("http://www.example.org/"), skyr::url("http://www.example.com/")};
