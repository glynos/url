...
export module skyr.v3.core.host;
...
export {
   ...
   public:
    ...
    [[nodiscard]] auto serialize() const {
      constexpr static auto serialize = [](auto &&host) -> std::string {
        ...
        if constexpr (std::is_same_v<T, skyr::ipv4_address>) {
          return host.serialize();
        } else if constexpr (std::is_same_v<T, skyr::ipv6_address>) {
          return std::format("[{}]", host.serialize());
        } else if constexpr (std::is_same_v<T, skyr::domain_name> || std::is_same_v<T, skyr::opaque_host>) {
          return host.name;
        } else {
          return std::string();
        }
  ...
  inline auto parse_opaque_host(std::string_view input, bool *validation_error)
    ...
    return skyr::opaque_host{input | std::ranges::views::transform(pct_encode) | ranges::actions::join};
  }
  }  // namespace details
  ...
  inline auto parse_host(std::string_view input, bool is_not_special, bool *validation_error)
    ...
    if (!host) {
      if (host.error() == ipv4_address_errc::overflow) {
        ...
        return skyr::host{skyr::domain_name{std::move(ascii_domain)}};
      }
    }
    *validation_error = ipv4_validation_error;
    return skyr::host{host.value()};
  }
