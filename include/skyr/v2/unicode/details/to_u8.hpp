...
template<class Source> requires is_string_convertible<Source, char>
inline auto to_u8(const Source &source) -> tl::expected<std::string, unicode::unicode_errc> {
  return std::string(source);
}

template<class Source> requires is_string_convertible<Source, char8_t>
inline auto to_u8(const Source &source) -> tl::expected<std::string, unicode::unicode_errc> {
  return std::string(std::cbegin(source), std::cend(source));
}

template<class Source> requires is_string_convertible<Source, char16_t> || is_string_convertible<Source, wchar_t>
inline auto to_u8(const Source &source) -> tl::expected<std::string, unicode::unicode_errc> {
  return unicode::as<std::string>(unicode::views::as_u16(source) | unicode::transforms::to_u8);
}

template<class Source> requires is_string_convertible<Source, char32_t>
inline auto to_u8(const Source &source) -> tl::expected<std::string, unicode::unicode_errc> {
