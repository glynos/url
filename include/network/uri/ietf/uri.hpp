...
#ifndef NETWORK_IETF_URI_URI_INC
#define NETWORK_IETF_URI_URI_INC
...
namespace std {
template <>
struct hash<::network::ietf::uri> {
  std::size_t operator()(const ::network::ietf::uri &uri_) const {
    std::size_t seed = 0;
    std::for_each(std::begin(uri_), std::end(uri_),
                  [&seed](network::ietf::uri::value_type v) {
                    std::hash<::network::ietf::uri::value_type> hasher;
...
#endif  // NETWORK_IETF_URI_URI_INC
