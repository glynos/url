...
TEST(uri_normalization_test, string_comparison) {
  ...
  ASSERT_EQ(
      ...
      instance.normalize(network::ietf::uri_comparison_level::string_comparison).string());
}
...
TEST(uri_normalization_test, path_depth_below_root) {
  network::uri instance("http://www.example.com/..");
  ASSERT_THROW(instance.normalize(network::ietf::uri_comparison_level::syntax_based),
               std::system_error);
}
