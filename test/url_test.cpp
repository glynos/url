...
TEST(whatwg_url_test, git_scheme_is_not_special) {
  network::whatwg::url instance("git://example.com");
  ASSERT_FALSE(instance.is_special());
}

TEST(whatwg_url_test, http_default_port_is_80) {
  auto port = network::whatwg::url::default_port("http");
  ASSERT_TRUE(port);
  ASSERT_EQ(80, *port);
}

TEST(whatwg_url_test, https_default_port_is_443) {
  auto port = network::whatwg::url::default_port("https");
  ASSERT_TRUE(port);
  ASSERT_EQ(443, *port);
}

TEST(whatwg_url_test, file_default_port_doesnt_exist) {
  auto port = network::whatwg::url::default_port("file");
  ASSERT_FALSE(port);
}

TEST(whatwg_url_test, git_default_port_is_not_given) {
  auto port = network::whatwg::url::default_port("git");
  ASSERT_FALSE(port);
}
